...
;
(function (ionic) {
  var readyCallbacks = [], domReady = function () {
...
(function (ionic) {
  ...
  var platformName, platformVersion, readyCallbacks = [];
...
(function (window, document, ionic) {
  ...
  function tapPolyfill(orgEvent) {
    if (!orgEvent.gesture || !orgEvent.gesture.srcEvent)
      return;
    var e = orgEvent.gesture.srcEvent;
    var ele = e.target;
    if (isRecentTap(e)) {
    ...
    while (ele) {
      if (ele.tagName === 'INPUT' || ele.tagName === 'A' || ele.tagName === 'BUTTON' || ele.tagName === 'TEXTAREA' || ele.tagName === 'SELECT') {
    ...
    if (ele && (ele.tagName === 'INPUT' || ele.tagName === 'TEXTAREA' || ele.tagName === 'SELECT')) {
      ele.blur();
    }
  }
  ...
  function isRecentTap(event) {
    var tapId, existingCoordinates, currentCoordinates, hitRadius = 15;
    for (tapId in tapCoordinates) {
      existingCoordinates = tapCoordinates[tapId];
      if (!currentCoordinates)
        currentCoordinates = getCoordinates(event);
      if (currentCoordinates.x > existingCoordinates.x - hitRadius && currentCoordinates.x < existingCoordinates.x + hitRadius && currentCoordinates.y > existingCoordinates.y - hitRadius && currentCoordinates.y < existingCoordinates.y + hitRadius) {
        return true;
      }
    }
  }
  ...
  function getCoordinates(event) {
    ...
    if (gesture) {
      ...
      var e = gesture.changedTouches && gesture.changedTouches[0] || gesture.originalEvent && gesture.originalEvent.changedTouches && gesture.originalEvent.changedTouches[0] || touches[0].originalEvent || touches[0];
      if (e)
        return {
          x: e.clientX,
          y: e.clientY
  ...
  var tapCoordinates = {};
  var CLICK_PREVENT_DURATION = 450;
...
(function (ionic) {
  ...
  function androidKeyboardFix() {
    ...
    function resize() {
      ...
      } else if (rememberedDeviceHeight !== window.innerHeight && window.innerHeight < rememberedDeviceHeight) {
      ...
      function handleKeyboardChange() {
        ...
        if (activeEl) {
          ionic.trigger('scrollChildIntoView', { target: activeEl }, true);
        }
      }
    }
  }
...
;
angular.module('ionic.decorator.location', []).config([
  '$provide',
  function ($provide) {
...
(function () {
  'use strict';
  angular.module('ionic.ui.service.scrollDelegate', []).factory('$ionicScrollDelegate', [
    '$rootScope',
    '$timeout',
    '$q',
    '$anchorScroll',
    '$location',
    '$document',
    function ($rootScope, $timeout, $q, $anchorScroll, $location, $document) {
      return {
...
(function () {
  'use strict';
  angular.module('ionic.ui.service.sideMenuDelegate', []).factory('$ionicSideMenuDelegate', [
    '$rootScope',
    '$timeout',
    '$q',
...
(function () {
  'use strict';
  angular.module('ionic.ui.service.slideBoxDelegate', []).factory('$ionicSlideBoxDelegate', [
    '$rootScope',
    '$timeout',
...
;
angular.module('ionic.service.loading', ['ionic.ui.loading']).factory('$ionicLoading', [
  '$rootScope',
  '$document',
  '$compile',
...
angular.module('ionic.service.modal', [
  ...
  '$ionicPlatform',
  '$ionicTemplateLoader',
  function ($rootScope, $document, $compile, $animate, $q, $timeout, $ionicPlatform, $ionicTemplateLoader) {
    var ModalView = ionic.views.Modal.inherit({
        ...
        remove: function () {
          var self = this, element = angular.element(this.el);
...
;
angular.module('ionic.service.popup', ['ionic.service.templateLoad']).factory('$ionicPopup', [
  '$rootScope',
  '$document',
  '$compile',
  'TemplateLoader',
...
;
angular.module('ionic.service.templateLoad', []).factory('$ionicTemplateLoader', [
  '$q',
  '$http',
  '$templateCache',
...
;
angular.module('ionic.service.view', [
  'ui.router',
  'ionic.service.platform'
]).run([
  '$rootScope',
  ...
  function ($rootScope, $state, $location, $window, $injector) {
    ...
    return {
      register: function (containerScope, element) {
        var viewHistory = $rootScope.$viewHistory, currentStateId = this.getCurrentStateId(), hist = this._getHistory(containerScope), currentView = viewHistory.currentView, backView = viewHistory.backView, forwardView = viewHistory.forwardView, rsp = {
            viewId: null,
            navAction: null,
            navDirection: null,
      ...
      clearHistory: function () {
        var historyId, x, view, histories = $rootScope.$viewHistory.histories, currentView = $rootScope.$viewHistory.currentView;
...
(function (ionic) {
  'use strict';
  angular.module('ionic.ui.header', ['ngAnimate']).directive('barHeader', [
    '$ionicScrollDelegate',
    ...
    function ($ionicScrollDelegate) {
      return {
        ...
        replace: true,
        transclude: true,
        template: '<header class="bar bar-header">                <div class="buttons">                  <button ng-repeat="button in leftButtons" class="button no-animation" ng-class="button.type" ng-click="button.tap($event, $index)" bind-html-unsafe="button.content">                  </button>                </div>                <h1 class="title" bind-html-unsafe="title"></h1>                <div class="buttons">                  <button ng-repeat="button in rightButtons" class="button no-animation" ng-class="button.type" ng-click="button.tap($event, $index)" bind-html-unsafe="button.content">                  </button>                </div>              </header>',
        scope: {
          leftButtons: '=',
          rightButtons: '=',
          title: '=',
...
;
angular.module('ionic.ui.bindHtml', []).directive('bindHtmlUnsafe', function () {
  return function (scope, element, attr) {
...
(function () {
  ...
  angular.module('ionic.ui.checkbox', []).directive('checkbox', function () {
    return {
      ...
      transclude: true,
      template: '<div class="item item-checkbox disable-pointer-events">' + '<label class="checkbox enable-pointer-events">' + '<input type="checkbox" ng-model="ngModel" ng-value="ngValue" ng-change="ngChange()">' + '</label>' + '<div class="item-content" ng-transclude></div>' + '</div>',
      compile: function (element, attr) {
...
(function () {
  ...
  angular.module('ionic.ui.content', [
    ...
    '$ionicScrollDelegate',
    '$controller',
    function ($parse, $timeout, $ionicScrollDelegate, $controller) {
      return {
        restrict: 'E',
        replace: true,
        ...
        compile: function (element, attr, transclude) {
          ...
          function prelink($scope, $element, $attr, navViewCtrl) {
            var clone, sc, scrollView, scrollCtrl, c = angular.element($element.children()[0]);
...
(function () {
  ...
  angular.module('ionic.ui.scroll', []).directive('scroll', [
    ...
    '$timeout',
    '$controller',
    function ($parse, $timeout, $controller) {
      return {
        restrict: 'E',
        replace: true,
...
angular.module('ionic.ui.tabs', [
  ...
  '$parse',
  '$interpolate',
  function ($ionicViewService, $rootScope, $parse, $interpolate) {
    return {
      restrict: 'E',
      require: '^tabs',
      scope: true,
      ...
      compile: function (element, attr, transclude) {
        return function link($scope, $element, $attr, tabsCtrl) {
          ...
          $rootScope.$on('$stateChangeSuccess', function (value) {
            if ($ionicViewService.isCurrentStateNavView($scope.navViewName) && $scope.tabIndex !== tabsCtrl.selectedIndex) {
              tabsCtrl.select($scope.tabIndex);
            }
          });
        };
  ...
  return {
    ...
    replace: true,
    scope: true,
    template: '<div class="tabs">' + '<tab-controller-item icon-title="{{c.title}}" icon="{{c.icon}}" icon-on="{{c.iconOn}}" icon-off="{{c.iconOff}}" badge="c.badge" badge-style="c.badgeStyle" active="c.isVisible" index="$index" ng-repeat="c in controllers"></tab-controller-item>' + '</div>',
    link: function ($scope, $element, $attr, tabsCtrl) {
      $element.addClass($scope.tabsType);
      $element.addClass($scope.tabsStyle);
    }
  };
}).directive('tabControllerItem', [
  '$window',
  function ($window) {
    return {
      restrict: 'E',
      ...
      template: '<a ng-class="{active:active, \'has-badge\':badge}" ng-click="selectTab()" class="tab-item">' + '<i class="badge {{badgeStyle}}" ng-if="badge">{{badge}}</i>' + '<i class="icon {{icon}}" ng-if="icon"></i>' + '<i class="{{iconOn}}" ng-if="active"></i>' + '<i class="{{iconOff}}" ng-if="!active"></i>' + '<span bind-html-unsafe="iconTitle"></span>' + '</a>'
    };
  }
]);
;
(function (ionic) {
  ...
  angular.module('ionic.ui.toggle', []).directive('toggle', function () {
    return {
      ...
      transclude: true,
      template: '<div class="item item-toggle disable-pointer-events">' + '<div ng-transclude></div>' + '<label class="toggle enable-pointer-events">' + '<input type="checkbox" ng-model="ngModel" ng-value="ngValue" ng-change="ngChange()" ng-disabled="ngDisabled">' + '<div class="track disable-pointer-events">' + '<div class="handle"></div>' + '</div>' + '</label>' + '</div>',
      compile: function (element, attr) {
        var input = element.find('input');
        if (attr.name)
          input.attr('name', attr.name);
...
(function (angular, ionic) {
  'use strict';
  angular.module('ionic.ui.touch', []).config([
    '$provide',
    function ($provide) {
      $provide.decorator('ngClickDirective', [
        '$delegate',
...
(function () {
  'use strict';
  angular.module('ionic.ui.viewState', [
    'ionic.service.view',
    'ionic.service.gesture'
  ]).directive('navBar', [
    '$ionicViewService',
    ...
    function ($ionicViewService, $rootScope, $animate, $compile) {
      ...
      return {
        ...
        template: '<header class="bar bar-header nav-bar invisible">' + '<div class="buttons"> ' + '<button view-back class="back-button button hide" ng-if="enableBackButton"></button>' + '<button ng-click="button.tap($event)" ng-repeat="button in leftButtons" class="button no-animation {{button.type}}" bind-html-unsafe="button.content"></button>' + '</div>' + '<h1 class="title" bind-html-unsafe="currentTitle"></h1>' + '<div class="buttons" ng-if="rightButtons.length"> ' + '<button ng-click="button.tap($event)" ng-repeat="button in rightButtons" class="button no-animation {{button.type}}" bind-html-unsafe="button.content"></button>' + '</div>' + '</header>',
        compile: function (tElement, tAttrs) {
          var backBtnEle = tElement[0].querySelector('.back-button');
          if (backBtnEle) {
            if (tAttrs.backButtonType)
              backBtnEle.className += ' ' + tAttrs.backButtonType;
...
(function () {
  'use strict';
  angular.module('ionic.ui.scroll').controller('$ionicScroll', [
    '$scope',
    'scrollViewOptions',
    '$timeout',
    '$ionicScrollDelegate',
    '$window',
    function ($scope, scrollViewOptions, $timeout, $ionicScrollDelegate, $window) {
      scrollViewOptions.bouncing = angular.isDefined(scrollViewOptions.bouncing) ? scrollViewOptions.bouncing : !ionic.Platform.isAndroid();
      var self = this;
